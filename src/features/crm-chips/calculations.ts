import type { Channel, Clinic, Contingency, ClinicSummary } from "./types";

export function contingencyScore(c: Contingency): number {
  const items = [
    c.has_backup_number,
    c.has_backup_api,
    c.has_approved_template,
    c.has_backup_chip,
    c.has_backup_device,
  ];
...
export function communicabilityScore(channels: Channel[], contingency?: Contingency): number {
  ...
  const reserva = channels.find(
    (c) => c.channel_type === "numero_reserva" || c.channel_type === "api_reserva",
  );
...
export function clinicHealth(channels: Channel[]): "normal" | "atencao" | "critico" {
  if (
    channels.some(
      (c) => c.status === "critico" || c.status === "bloqueado" || c.status === "desconectado",
    )
  )
    return "critico";

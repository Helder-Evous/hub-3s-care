  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-2 pb-3">
        ...
        <NewActivityModal clinicId={clinicId} leadId={leadId} patientId={patientId} />
      </CardHeader>
      <CardContent>
        {rows.length === 0 ? (
            ...
            {rows.map((a) => (
              <li key={a.id} className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <StatusBadge tone="muted">{ACTIVITY_TYPE_LABELS[a.activity_type]}</StatusBadge>
                  <p className="mt-1 text-sm text-foreground">
                    {a.summary ?? <span className="text-muted-foreground">Sem descrição</span>}
                  </p>
                </div>

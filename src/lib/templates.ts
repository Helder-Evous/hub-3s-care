import { templates, apiVolumes, type MetaTemplate, type ApiVolume, type TemplateCategory, type TemplateStatus } from "./mock-data";
...
export function templateAlerts(t: MetaTemplate): string[] {
  ...
  if (t.previous_category && t.previous_category !== t.category) {
    out.push(`Categoria mudou de ${templateCategoryLabel[t.previous_category]} para ${templateCategoryLabel[t.category]}`);
  }
...
export function clinicTemplateSummary(clinicId: string) {
...
export function volumeAlerts(v: ApiVolume): { msg: string; tone: "warning" | "critical" }[] {
  ...
  const hoursSinceSent = v.last_sent_at ? (Date.now() - new Date(v.last_sent_at).getTime()) / 3_600_000 : 999;
  const hoursSinceReceived = v.last_received_at ? (Date.now() - new Date(v.last_received_at).getTime()) / 3_600_000 : 999;
  if (hoursSinceSent > 4) out.push({ msg: `Sem envios há ${Math.floor(hoursSinceSent)}h`, tone: hoursSinceSent > 12 ? "critical" : "warning" });
  if (hoursSinceReceived > 4) out.push({ msg: `Sem recebimentos há ${Math.floor(hoursSinceReceived)}h`, tone: hoursSinceReceived > 12 ? "critical" : "warning" });
  const dr = deliveryRate(v);
  if (v.sent_today > 0 && dr < 90) out.push({ msg: `Taxa de entrega ${dr}% (abaixo do padrão)`, tone: dr < 70 ? "critical" : "warning" });
  if (v.avg_daily_sent_7d > 0 && v.sent_today > 0 && v.sent_today < v.avg_daily_sent_7d * 0.5) {
    out.push({ msg: `Volume ${Math.round((1 - v.sent_today / v.avg_daily_sent_7d) * 100)}% abaixo da média 7d`, tone: "warning" });
  }
...
export function clinicVolumeForApi(clinicId: string): ApiVolume | undefined {
